...
const PostShare = ({ isFixedShare, detailData }) => {
  const [shareWrapStyle, setShareWrapStyle] = useState({
    position: '',
    top: '',
    left: '',
  });
  ...
  function sendLinkCustom() {
    const { id, title, body } = detailData;
    let { thumbnail } = detailData;

    if (thumbnail === undefined) {
      thumbnail = 'default.png';
    }

    const description = removeHTMLTagFromString(parse(body));
    ...
    try {
      window.Kakao.Link.sendDefault({
        ...
        content: {
          ...
          imageUrl: `${thumbnail}`,
          link: {
            mobileWebUrl: `http://localhost:3000/detail/${id}`,
            webUrl: `http://localhost:3000/detail/${id}`,
          },
        },
        buttons: [
          {
  ...
  const { position, top, left } = shareWrapStyle;

  return (
    <S.ShareContainer onClick={sendLinkCustom}>
      <S.ShareWrap position={position} top={top} left={left}>
        <S.ShareBtn>
          <S.ShareImg
